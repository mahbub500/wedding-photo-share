...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "wps_{$_args['post_type']}", 'wps' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "wps_{$_args['post_type']}", $posts, 'wps', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'wps' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'wps_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /weadding-photo-share directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with weadding-photo-share_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'wps_template_overwrite_dir', get_stylesheet_directory() . '/weadding-photo-share/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( WEADDING_PHOTO_SHARE ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'weadding-photo-share' );
		}
	}
}